    ...
    <header className="w-[85%] mx-auto py-12">
      <nav className="flex justify-between items-center">
        ...
        <div className="hamburger sm:hidden block">
          <GiHamburgerMenu
            className="text-white text-3xl z-[999]"
        ...
        <ul
          className={`flex items-center ${
            navOpen
              ? "fixed top-0 left-0 right-0 bottom-0 flex-col bg-[#0A2640] w-full h-[100%] justify-center duration-200"
              : "hidden"
          }`}
        >
          <div className="hamburger sm:hidden block absolute top-0 right-0 m-10">
            <AiOutlineClose
              className="text-white text-5xl z-[999]"
              onClick={() => setNavOpen(!navOpen)}
            />
          </div>
          {navItems.map((item) => (
